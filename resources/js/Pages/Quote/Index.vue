    ...
    <a-layout-content>
      <a-space direction="vertical" :style="{ width: '100%' }">
        <a-form name="nest-messages" :model="searchform" layout="inline" @finish="search">
          <a-form-item>
            <a-input placeholder="Search by Quote Number" :allow-clear="true" v-model:value="searchform.term">
            </a-input>
          </a-form-item>
          <a-form-item>
        ...
        <a-table :columns="columns" :data-source="quotes">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'quote_number'">
      ...
      <a-drawer v-model:visible="create_form_visible" class="sales-bill" :size="'large'" style="color: red;"
        title="New Quote" placement="right" @after-visible-change="afterVisibleChange">
        <a-form :model="form" v-bind="layout" name="nest-messages" :validate-messages="validateMessages" @finish="submit"
          ...
          <a-row :gutter="24">
            ...
            <a-col class="gutter-row" span="8">
              <a-form-item name="name" label="Quote Number" :rules="[{ required: true }]">
            ...
            <a-col class="gutter-row" span="8">
              <a-form-item name="name" label="Quote Date" :rules="[{ required: true }]">
                <a-input v-model:value="form.name" />
              </a-form-item>
            </a-col>
            <a-col class="gutter-row" span="8">
              <a-form-item name="name" label="Expiry Date">
                <a-input v-model:value="form.name" />
              </a-form-item>
            </a-col>
          ...
          <a-table :columns="quotecolumns" :data-source="createQuoteItems" :pagination="false">
            <template #bodyCell="{ column, record }">
      ...
      <a-drawer v-model:visible="showdetail" class="custom-class" title="QT-000001" :size="'large'" placement="right"
        ...
        <a-space style="margin-bottom: 10px;">
          <a-button @click="onClose"> <EditOutlined /></a-button>
          <a-button type="primary" @click="onClose"><DeleteOutlined /></a-button>
          <a-dropdown>
            <template #overlay>
          ...
          <a-dropdown>
            ...
            <a-button>
              <InteractionOutlined /> Convert To
            </a-button>
          </a-dropdown>
          <a-button @click="onClose"><MailOutlined /> Send Quote</a-button>
          <a-button @click="onClose"><LikeOutlined /> Mark As Sent</a-button>
        </a-space>
        <div class="quote-template">
          <table style="width:100%" class="quote-table">
            ...
            <tr class="highlight">
              <td colspan="2">
                <p>Quote Number : QT-000001</p>
                <p>Quote Date : 10/10/2023</p>
                <p>Due Date : 10/10/2023</p>
              </td>
              <td colspan="2" align="right">
                <h4>INR 54000.00</h4>
            ...
            <tr>
              <td colspan="2">
                <div style="margin-right:20px">
                  ...
                  <p>1. Payment should be made within 30 days from the date of invoice.</p>
...
//adding 5 more data with different quote_number and customer_name
{
  key: '2',
  date: '10-10-2023',
  quote_number: 'quote-0002',
  reference_number: '',
  customer_name: 'John Doe',
  status: 'Draft',
  amount: '0.00',
...
// defining create quote items const
...
export default ({
  components: { AuthenticatedLayout, DownloadOutlined, PlusCircleOutlined, MoreOutlined, Head, Link, DownCircleOutlined, CloseCircleOutlined, EditOutlined,
  DeleteOutlined,
  MailOutlined,
  LikeOutlined,
  InteractionOutlined,FilePdfOutlined },
  props: {
  },
  setup(props) {
    return {
