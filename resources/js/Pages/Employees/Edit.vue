...
<template>
    ...
    <AuthenticatedLayout>
        <a-affix :offset-top="0">
            <a-page-header :ghost="false" :title="employee.name" @back="() => $inertia.visit(route('employees.index'))">
                <template #extra>
                    ...
                    <a-form :model="formState" v-bind="layout" name="nest-messages" :validate-messages="validateMessages"
                        ...
                        <a-form-item label="Timing" name="work_timing">
                            <a-time-range-picker :style="{ width: '100%' }" format="h:mm A"
                                v-model:value="formState.work_timing" value-format="HH:mm:ss" />
                        </a-form-item>
                        <h3 :wrapper-col="{ span: 12, offset: 6 }">
                        ...
                        <a-form-item label="Aadhar No" name="aadhar_number">
...
export default ({
    ...
    setup(props) {
        ...
        const formState = useForm({
            ...
            doj: props.employee.doj,
            address: props.employee.address,
            working_days: props.employee.working_days,
            work_timing: [props.employee.work_start_time, props.employee.work_end_time],
            bank_name: props.employee.bank_name,
            account_number: props.employee.account_number,
            ifsc_code: props.employee.ifsc_code,
            accountholder_name: props.employee.accountholder_name,
            employeeid: props.employee.employeeid,
            dob: props.employee.dob,
            aadhar_number: props.employee.aadhar_number,
            uan_number: props.employee.uan_number,
            pan_number: props.employee.pan_number,
            pf_number: props.employee.pf_number,
            esi_number: props.employee.esi_number,
        });
        return {
            formState,
            layout,
    ...
    methods: {
        ...
        submit() {
            this.formState.put(route("employees.update", this.employee.id));
        },
    },
});
</script>