  ...
  <AuthenticatedLayout>
    <a-layout-content>
      <a-space direction="vertical" :style="{ width: '100%' }">
        ...
        <a-table :columns="columns" :data-source="job_orders">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'job_order_number'">
      ...
      <a-drawer v-model:visible="create_form_visible" class="sales-bill" :size="'large'" style="color: red;" width="60%"
        title="Add order" placement="right" @after-visible-change="afterVisibleChange">
        <a-form :model="form" v-bind="layout" name="nest-messages" :validate-messages="validateMessages" @finish="submit"
          ...
          <a-row :gutter="24">
            <a-col class="gutter-row" span="12">
              <a-form-item name="client" label="Client" :rules="[{ required: true }]">
                <a-select v-model:value="form.tax" show-search placeholder="Select client" style="width: 100%"
                  :default-active-first-option="false" :options="customers" :not-found-content="value"></a-select>
              </a-form-item>
            </a-col>
            <a-col class="gutter-row" span="12">
              <a-form-item name="external_order_id" label="External Order ID" :rules="[{ required: true }]">
                <a-input v-model:value="form.name" />
              </a-form-item>
            </a-col>
            <a-col class="gutter-row" span="12">
              <a-form-item name="Date" label="Due date">
                <a-input v-model:value="form.name" />
              </a-form-item>
            </a-col>
          </a-row>
          <h4 class="t1">Items Info</h4>
          <a-table :columns="job_ordercolumns" :data-source="createPurchaseOrderItems" :pagination="false">
            <template #bodyCell="{ column, record }">
          ...
          <br />
          <br />
          <a-form-item name="name" label="Notes" :rules="[{ required: true }]">
